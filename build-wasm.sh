...
cmake \
    ...
    -DCMAKE_EXE_LINKER_FLAGS="-O3 --memory-init-file 0 -s USE_PTHREADS=1 -s PTHREAD_POOL_SIZE=8 -s NO_FILESYSTEM=1 -s 'EXPORTED_FUNCTIONS=[\"_argon2_hash\",\"_argon2_hash_ext\",\"_argon2_verify\",\"_argon2_error_message\",\"_malloc\",\"_free\"]' -s 'EXTRA_EXPORTED_RUNTIME_METHODS=[\"UTF8ToString\",\"allocate\",\"ALLOC_NORMAL\"]' -s DEMANGLE_SUPPORT=0 -s ASSERTIONS=0 -s NO_EXIT_RUNTIME=1 -s TOTAL_MEMORY=16MB -s BINARYEN_MEM_MAX=2147418112 -s ALLOW_MEMORY_GROWTH=1 -s WASM=1" \
    .
cmake --build .
